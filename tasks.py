...
def coverage(c):
    ...
    """
    c.run("coverage run --source django-telnyx runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/django-telnyx.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ django_telnyx")
...
def lint(c):
    ...
    """
    c.run("flake8 django-telnyx tests")
...
def release(c, bumpsize=''):
    ...
    import django_telnyx
    c.run("python setup.py sdist bdist_wheel")
    c.run("twine upload dist/*")

    c.run('git tag -a {version} -m "New version: {version}"'.format(version=django_telnyx.__version__))
    c.run("git push --tags")
